...
# ============================================================================
# Deterministic actuator moment allocation (opt.deterministic=True)
#
# Replaces the racy `wp.atomic_add(moment_nnz, worldid, N)` slot allocation in
# _transmission with a deterministic count->scan->emit pipeline so that
# d.moment_rowadr lands at the same position on every run of the same input.
# ============================================================================


@wp.kernel
def _transmission_count(
  # Model:
  nv: int,
  jnt_type: wp.array[int],
  body_weldid: wp.array[int],
  ...
  if trntype == TrnType.JOINT or trntype == TrnType.JOINTINPARENT:
    ...
    b1 = body_weldid[site_bodyid[trnid[0]]]
    b2 = body_weldid[site_bodyid[trnid[1]]]
    da1 = int(-1)
    da2 = int(-1)
    if b1 > 0:
      da1 = body_dofadr[b1] + body_dofnum[b1] - 1
    if b2 > 0:
      da2 = body_dofadr[b2] + body_dofnum[b2] - 1
    ...
    if refid == -1:
      ...
      da = int(-1)
      if b1 > 0:
        da = body_dofadr[b1] + body_dofnum[b1] - 1
      while da >= 0:
        nnz += 1
        da = dof_parentid[da]
    else:
      b0 = body_weldid[site_bodyid[siteid]]
      b1 = body_weldid[site_bodyid[refid]]
      da1 = int(-1)
      da2 = int(-1)
      if b0 > 0:
        da1 = body_dofadr[b0] + body_dofnum[b0] - 1
      if b1 > 0:
        da2 = body_dofadr[b1] + body_dofnum[b1] - 1
      while da1 >= 0 or da2 >= 0:
        da = wp.max(da1, da2)
  ...
  nnz_count_out[worldid, actid] = nnz


@wp.kernel
def _moment_per_world_exclusive_scan(
  # In:
  counts_in: wp.array2d[int],
  # Out:
  offsets_out: wp.array2d[int],
):
  worldid = wp.tid()
  acc = int(0)
  n = counts_in.shape[1]
...
def _transmission(deterministic: bool):
  ...
  def kernel(
    ...
    moment_offsets_in: wp.array2d[int],
    # Data out:
    ...
    if trntype == TrnType.JOINT or trntype == TrnType.JOINTINPARENT:
      ...
      if jnt_typ == JointType.FREE:
        ...
        if wp.static(deterministic):
          rowadr = moment_offsets_in[worldid, actid]
        else:
      ...
      if wp.static(deterministic):
        rowadr_mom = moment_offsets_in[worldid, actid]
      else:
...
def transmission(m: Model, d: Data):
  ...
  if det and m.nu > 0:
    nnz_counts = wp.empty((d.nworld, m.nu), dtype=int)
    wp.launch(
      _transmission_count,
      dim=(d.nworld, m.nu),
      inputs=[
    ...
    moment_offsets = wp.empty((d.nworld, m.nu), dtype=int)
    wp.launch(
      _moment_per_world_exclusive_scan,
      dim=d.nworld,
      inputs=[nnz_counts],
      outputs=[moment_offsets],
    )
  else:
    moment_offsets = wp.zeros((1, 1), dtype=int)
